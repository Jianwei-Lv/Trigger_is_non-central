...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask=None,scale_factor=True,attn_=None):
...
class PosEncoding(nn.Module):
    def __init__(self, max_seq_len, d_word_vec):
        ...
        self.pos_enc.weight = nn.Parameter(torch.from_numpy(pos_enc), requires_grad=True)

    def forward(self, input_len):
        max_len = torch.max(input_len)
        tensor = torch.cuda.LongTensor if input_len.is_cuda else torch.LongTensor
        input_len=input_len.cpu().tolist()
        max_len=max_len.cpu().tolist()
