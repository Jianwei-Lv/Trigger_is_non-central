...
from data_load import ACE2005Dataset, pad, all_entities, all_postags, all_arguments,idx2events
...
def eval(model, iterator, fname):
    ...
    words_all, triggers_all, triggers_hat_all, arguments_all, arguments_hat_all ,sentence_events= [], [], [], [], [],[]
    with torch.no_grad():
        for i, batch in enumerate(iterator):
            ...
            trigger_logits, triggers_y_2d, trigger_hat_2d, argument_hidden, argument_keys ,event_logits,event_sigmoid ,sent_trigger_y_2d,sentence_event= model.module.predict_triggers(tokens_x_2d=tokens_x_2d, entities_x_3d=entities_x_3d,
                                                                                                                          ...
                                                                                                                          triggers_y_2d=triggers_y_2d, arguments_2d=arguments_2d,glove_id_2d=glove_id_2d ,epoch=1,sent_trigger_y_2d = sent_trigger_y_2d,position_2d=position_2d,seqlens_1d=seqlens_1d)

            words_all.extend(words_2d)
            triggers_all.extend(triggers_2d)
            trigger_hat_2d=trigger_hat_2d.cpu().numpy().tolist()
            # if len(trigger_hat_2d)>8:
            #     triggers_hat_all.append(trigger_hat_2d)
            # else:
            triggers_hat_all.extend(trigger_hat_2d)
            arguments_all.extend(arguments_2d)
            sentence_events.extend(sentence_event)
    ...
    with open('temp', 'w',encoding='utf-8') as fout:
        for i, (words, triggers, triggers_hat, arguments, arguments_hat,sen_event) in enumerate(zip(words_all, triggers_all, triggers_hat_all, arguments_all, arguments_hat_all,sentence_events)):

            triggers_hat = triggers_hat[:len(words)]
            triggers_hat = [idx2events[hat] for hat in triggers_hat]

            # [(ith sentence, t_start, t_end, t_type_str)]
            triggers_true.extend([(i, *item) for item in find_triggers(triggers)])
            triggers_pred.extend([(i, *item) for item in find_triggers(triggers_hat,sen_event=sen_event)])
